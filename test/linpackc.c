...
/*     compute a residual to verify results.  */ 
	...
	fprintf(stderr,"      ratio\n");
        ...
        xtime[2][0] = total;
        xtime[3][0] = ops/(1.0e3*total);
        xtime[4][0] = 2.0e3/xtime[3][0];
        xtime[5][0] = total/cray;
	...
	print_time(0);
        ...
        dgesl((REAL *)a,lda,n,ipvt,b,0);
        xtime[1][1] = second() - t1;
        total = xtime[0][1] + xtime[1][1];
        xtime[2][1] = total;
        xtime[3][1] = ops/(1.0e3*total);
        xtime[4][1] = 2.0e3/xtime[3][1];
        xtime[5][1] = total/cray;
        ...
        dgesl((REAL *)a,lda,n,ipvt,b,0);
        xtime[1][2] = second() - t1;
        total = xtime[0][2] + xtime[1][2];
        xtime[2][2] = total;
        xtime[3][2] = ops/(1.0e3*total);
        xtime[4][2] = 2.0e3/xtime[3][2];
        xtime[5][2] = total/cray;
	...
	for (i = 0; i < ntimes; i++) {
        ...
        xtime[0][3] = (second() - t1 - tm2)/ntimes;
        t1 = second();
	...
	for (i = 0; i < ntimes; i++) {
        ...
        xtime[1][3] = (second() - t1)/ntimes;
        total = xtime[0][3] + xtime[1][3];
        xtime[2][3] = total;
        xtime[3][3] = ops/(1.0e3*total);
        xtime[4][3] = 2.0e3/xtime[3][3];
        xtime[5][3] = total/cray;
	...
	print_time(3);
        ...
        xtime[1][4] = second() - t1;
        total = xtime[0][4] + xtime[1][4];
        xtime[2][4] = total;
        xtime[3][4] = ops/(1.0e3*total);
        xtime[4][4] = 2.0e3/xtime[3][4];
        xtime[5][4] = total/cray;
        ...
        dgesl((REAL *)aa,ldaa,n,ipvt,b,0);
        xtime[1][5] = second() - t1;
        total = xtime[0][5] + xtime[1][5];
        xtime[2][5] = total;
        xtime[3][5] = ops/(1.0e3*total);
        xtime[4][5] = 2.0e3/xtime[3][5];
        xtime[5][5] = total/cray;
	...
	dgesl((REAL *)aa,ldaa,n,ipvt,b,0);
	xtime[1][6] = second() - t1;
	total = xtime[0][6] + xtime[1][6];
	xtime[2][6] = total;
	xtime[3][6] = ops/(1.0e3*total);
	xtime[4][6] = 2.0e3/xtime[3][6];
	xtime[5][6] = total/cray;
	...
	xtime[0][7] = (second() - t1 - tm2)/ntimes;
	t1 = second();
	for (i = 0; i < ntimes; i++) {
		dgesl((REAL *)aa,ldaa,n,ipvt,b,0);
	}
	xtime[1][7] = (second() - t1)/ntimes;
	total = xtime[0][7] + xtime[1][7];
	xtime[2][7] = total;
	xtime[3][7] = ops/(1.0e3*total);
	xtime[4][7] = 2.0e3/xtime[3][7];
	xtime[5][7] = total/cray;

	/* the following code sequence implements the semantics of
	   the Fortran intrinsics "nint(min(xtime[3][3],xtime[3][7]))"	*/

	kf = (xtime[3][3] < xtime[3][7]) ? xtime[3][3] : xtime[3][7];
...
fprintf(stderr,"%11.2f%11.2f%11.2f%11.0f%11.2f%11.2f\n",   (double)xtime[0][row],
       (double)xtime[1][row], (double)xtime[2][row], (double)xtime[3][row], 
       (double)xtime[4][row], (double)xtime[5][row]);
}
