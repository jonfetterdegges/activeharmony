...
static unsigned long gla_mt[N1]; // the array for the state vector  
static int gl_mti=N1+1; // gl_mti==N+1 means gla_mt[N] is not initialized 
...
float genrand()
{
    ...
    if (gl_mti >= N1) { // generate N words at one time 
        int kk;

        if (gl_mti == N1+1)   // if sgenrand() has not been called,
        ...
        for (;kk<N1-1;kk++) {
            y = (gla_mt[kk]&UPPER_MASK)|(gla_mt[kk+1]&LOWER_MASK);
            gla_mt[kk] = gla_mt[kk+(M-N1)] ^ (y >> 1) ^ mag01[y & 0x1];
        }
        y = (gla_mt[N1-1]&UPPER_MASK)|(gla_mt[0]&LOWER_MASK);
        gla_mt[N1-1] = gla_mt[M-1] ^ (y >> 1) ^ mag01[y & 0x1];
